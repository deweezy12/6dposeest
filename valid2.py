...
def extract_tecture(filepath, iterator, corners):
    ...
    # Define the four points as tuples (x, y)
    point1_float = tuple(corners[2, :]) 
    point1 = tuple(int(i) for i in point1_float)
    point2_float = tuple(corners[4, :]) # before 2
    point2 = tuple(int(i) for i in point2_float)
    point3_float = tuple(corners[6, :]) # before 1
    point3 = tuple(int(i) for i in point3_float)
    point4_float = tuple(corners[8, :])
    ...
    #cv2.waitKey(0)
    output = "C:/python_work/singleshotpose/LINEMOD/legocolor/texture/" + str(iterator) + ".png"
