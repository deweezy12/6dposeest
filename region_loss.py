...
def build_targets(pred_corners, target, num_keypoints, num_anchors, num_classes, nH, nW, noobject_scale, object_scale, sil_thresh, seen):
    ...
    #print(f"conf_mask length {len(conf_mask)}")
    ...
    nAnchors = nA*nH*nW
    #nAchros = nA.item() * nH.item() * nW.item()
    #print(f"nAnchros are: {nAnchors}")
    nPixels  = nH*nW
    for b in range(nB):
        cur_pred_corners = pred_corners[b*nAnchors:(b+1)*nAnchors].t()
        cur_confs = torch.zeros(nAnchors)
        #print(f"cur_confs are size {cur_confs.size()}")
        #print(f"cur_confs are slen {len(cur_confs)}")
        #cur_confs = cur_confs.expand_as(conf_mask[b])
        for t in range(50):
...
class RegionLoss(nn.Module):
    def __init__(self, num_keypoints=4, num_classes=1, anchors=[], num_anchors=1, pretrain_num_epochs=15):
        print(f"the number of keypoints is: {num_keypoints}")
